...
function BIgWeather({location}) {
    ...
    useEffect(() => {
        ...
        axios.get(import.meta.env.VITE_WEATHER_URL,
                {
                    params: {
                        city: location,
  ...
  return (
    ...
    {count!==0 && <div className="flex justify-center">
        <div className="card  min-w-sm max-w-sm border border-gray-100 bg-green-600 transition-shadow test shadow-lg hover:shadow-shadow-xl w-full bg-green-600 text-purple-50 rounded-md">
            <h2 className="text-md mb-2 px-4 pt-4">
                <div className="flex justify-between">
                    <div className="badge relative top-0">
                        <span className="mt-2 py-1 h-12px text-md font-semibold w-12px  rounded right-1 bottom-1 px-4">{info.city_name}</span>
                    </div>
                </div>
            </h2>
            ...
            <div className="flex items-center p-4">
                <div className="flex justify-center items-center w-96">
                    <img src={`/icons/${info.weather.icon}.png`} className='h-1/4 w-1/4' alt="" />
                </div>
            </div>
            <div className="text-md pt-4 pb-4 px-4">
                <div className="flex justify-between items-center">
                    ...
                    <div>
                        <h1 className="text-6xl"> {info.temp}&deg; </h1>
                    </div>
                </div>
            </div>
        </div>
